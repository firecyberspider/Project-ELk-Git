...
output=$HOME/Documents/New_Server_Setup.txt
...
# Log file header
echo "Log file for general server setup script." >> $output
echo "############################" >> $output
echo "Log generated on: $(date)" >> $output
echo "############################" >> $output
echo "" >> $output
...
echo "$(date) Installed needed pacakges: ${packages[@]}" | tee -a $output
...
# Print it out and Log it
echo "$(date) Created sys_admin user. Password to be created upon login" | tee -a $output
...
# Print it out and Log it
echo "$(date) Disabled root shell. Root user cannot login." | tee -a $output
...
# Print it out and Log it
echo "$(date) Changed permissions on sensitive /etc files." | tee -a $output
...
# Print it out and Log it
echo "$(date) Added ~/scripts directory to sysadmin's PATH." | tee -a $output
...
# Print it out and Log it
echo "$(date) Added custom alias collection to sysadmin's bashrc." | tee -a $output

#Print out and log Exit
echo "$(date) Script Finished. Exiting." | tee -a $output
